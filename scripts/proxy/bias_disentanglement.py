...
Outputs (all in results/debiased_proxy/):
...
OUT_DIR = Path("results/debiased_proxy")
OUT_DIR.mkdir(parents=True, exist_ok=True)

TRANSFORMED_DIR = Path("results/transformed_proxy")
ARCH_DATA_DIR   = Path("data/chunks_clean/arch2infos")
...
def phase_e(corr_results: Dict, phase_b_summary: Dict):
    ...
    for name in PROXIES_TO_DEBIAS:
        ...
        print(f"  {name:<14}  {r['global_spearman_rho']:>9.3f}  "
              ...
              f"{r['decision']}")
    print("  " + "-" * 62)
    print("\n  Files written to results/debiased_proxy/:")
